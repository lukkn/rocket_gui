...
LJM Library:
    LJM Library Installer:
        https://labjack.com/suuupport/software/installers/ljm
    LJM Users Guide:
...
POS_IN_NAMES = ["AIN4", "AIN5","AIN6","AIN7"]
...
try:
    ...
    if deviceType == ljm.constants.dtT4:
        ...
        aNames = ["AIN4_RANGE", "AIN5_RANGE", "AIN6_RANGE","AIN7_RANGE","STREAM_SETTLING_US",
                  "STREAM_RESOLUTION_INDEX"]
        aValues = [2.5, 2.5, 2.5, 2.5, 0, 1]
    ...
    fileName = '/home/pi/logs/data_log_' + datetime.now().strftime("%Y-%m-%d_%H:%M:%S") + '.csv'
    #file = open(fileName, 'w')

    with open(fileName, 'w', newline='') as file:
        writer = csv.writer(file)
        if os.stat(fileName).st_size == 0:
            writer.writerow(["Time", "AIN4", "AIN5","AIN6","AIN7"])
        ...
        try:
            while True:
                ...
                for j in range(0, scansPerRead):
                    #readStr += "Time: %f, " % (time.time_ns() - epoch)
                    Time = datetime.now().strftime("%H:%M:%S.%f")

                    csv_row = [Time]
                    for k in range(0, NUM_IN_CHANNELS):
                        csv_row += [round(data[j * NUM_IN_CHANNELS + k], 5)]
