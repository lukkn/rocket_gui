...
SFE_ADS122C04 *ADCs[5];
SFE_ADS122C04 *mySensor = new SFE_ADS122C04;
...
void setup(void)
{
  ...
  for (int i = 0; i < 5; i++){
    ...
    ADCs[i] = new SFE_ADS122C04;
    //I added the adress parameters, not in orginal example
    if (ADCs[i]->begin((i + 0x40), Wire) == false) //Connect to the PT100 using the defaults: Address 0x45 and the Wire port
    {
      Serial.println(F("Qwiic PT100 not detected at default I2C address. Please check wiring. Freezing."));
    }
    ADCs[i]->enableDebugging(Serial);
  }

  for (int i = 0; i < 5; i++){
    if (ADCs[i] != NULL){
      ADCs[i]->setGain(ADS122C04_GAIN_1);
    }
  }
  ...
  for (int i = 0; i < 5; i++){
    if (ADCs[i] != NULL){
      Serial.print("ADC: ");
      Serial.println(i);
      ADCs[i]->printADS122C04config();
    }
  }

  ADCs[4]->setGain(ADS122C04_GAIN_2);
  ...
  */
  ...
  if (mySensor->begin(0x40, Wire) == false) //Connect to the PT100 using the defaults: Address 0x45 and the Wire port
    {
      Serial.println(F("Qwiic PT100 not detected at default I2C address. Please check wiring. Freezing."));
    }
  ...
  //myOtherSensor->enableDebugging(Serial); //Enable debug messages on Serial
  ...
  mySensor->setDataCounter(ADS122C04_DCNT_ENABLE); // Disable the data counter (Note: the library does not currently support the data count)
...
void loop()
{
  ...
  */
  ...
  Serial.println((double)raw_v.INT32/4096000.0);
